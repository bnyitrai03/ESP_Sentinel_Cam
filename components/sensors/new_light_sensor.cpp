...
NewLightSensor::NewLightSensor(I2CManager &i2c)
...
esp_err_t NewLightSensor::read_register(uint8_t reg, uint16_t *value) {
  esp_err_t err;
...
esp_err_t NewLightSensor::write_register(uint8_t reg, uint16_t value) {
  uint8_t write_buf[3];
  write_buf[0] = reg;
...
float NewLightSensor::get_value() const {
  return static_cast<int>(_light_value);
}